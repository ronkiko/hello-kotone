...
def main() -> None:
    ...
    require('position = Vector2(-110, -350)' in rig, "torso artwork must map its pelvis pivot to the root origin")
    require("auto_calculate_length_and_angle = false" in rig, "Torso bone must use explicit geometry")
    require("length = 300.0" in rig and "bone_angle = -1.570796" in rig, "Torso bone must point vertically upward")
    require('[node name="Art_torso" type="Sprite2D" parent="Skeleton2D/Torso"]' in rig, "torso art is not bound to Torso")
    require("rotation = 1.570796" not in rig, "art must not use a counter-rotation")
