...
def main() -> None:
    ...
    require("auto_calculate_length_and_angle = true" in rig, "Torso must derive its gizmo from the Head child joint")
    ...
    require("rotation = 1.570796" not in rig and "rotation = -1.570796" not in rig, "rig must not use compensating node rotations")
    ...
    require("transform = Transform2D(1, 0, 0, 1, 16, -328)" in rig, "Head current pose must match the neck joint")
