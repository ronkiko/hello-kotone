...
def main() -> None:
    ...
    require('[node name="Art_torso" type="Sprite2D" parent="Skeleton2D/Torso"]' in rig, "torso art must be a rigid child of Torso")
    require('[node name="Art_head" type="Sprite2D" parent="Skeleton2D/Torso/Head"]' in rig, "head art must be a rigid child of Head")
    require(rig.count("centered = false") == 2, "rigid sprites must use explicit source-pixel registration")
    require("position = Vector2(-110, -350)" in rig, "torso art must register its measured pelvis pivot at the Torso origin")
    require("position = Vector2(-80, -190)" in rig, "head art must register its measured neck pivot at the Head origin")
    require('type="Polygon2D"' not in rig, "single-bone rigid M03 parts must not use deforming polygon meshes")
